...
def F(x, s, z, mu, componentwise=False, debug=False):
    ...
    ## compute components
    lag = grad_f0(x) - z[0]*grad_f1(x) - z[1]*grad_f2(x)
    slk = S.dot(z).reshape(2,1) - mu*e
    con1 = x[0]*x[1] - 5. - s[0]
    con2 = 20. - x[0]**2 - x[1]**2 - s[1]
    con = np.array([con1, con2]).reshape(2,1)
...
def JF(x, s, z, mu, debug=False):
    ...
    j13 = np.vstack([grad_f1(x).T, grad_f2(x).T])            # wrt z1, z2 (not sure if .T is right)
