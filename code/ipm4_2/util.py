...
def visualize(xs, ss, lams, ms, xs_cp, mu):
    ## initialize contours
    delta = 0.025
    x1 = np.arange(-4.0, 2.0, delta)
    x2 = np.arange(-5.0, 15.0, delta)
    levels = range(4,11)
    levels += [15, 20, 25, 30, 40, 50, 70, 100, 200, 300]
...
def visualize_sims(xss, sss, lamss, mss, xss_cp, mus):
    ...
    plt.plot([], [], label="slack", color="black", alpha=1)
    plt.plot([], [], label="merit", color="black", alpha=0.6)
    plt.plot([], [], label="dual", color="black", alpha=0.3)
    plt.legend(loc=1, fontsize=5)
    plt.ylabel("dual")
    plt.title("s, lambda, and merit")
...
def objective(x, mu):
    x1, x2 = x[0], x[1]
    return x1**4 + x2**2 + x1**2 - 2.*x1 + 5. - mu*np.log(constraint(x))
