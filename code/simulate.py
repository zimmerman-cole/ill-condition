...
# ds = optimize.DecompositionSolver(A=A,b=b,full_output=True)
# x_opt,i,r,e = ds.solve(x_0=x_0, x_true=x_true, max_iter=500, d_type="lu")
# print(la.norm(np.dot(A,x_opt)-b))

# gds = optimize.GradientDescentSolver(A=A,b=b,full_output=True)
# x_opt,i,r,e = gds.solve(x_0=x_0, x_true=x_true, max_iter=500, d_type="lu")
# print(la.norm(np.dot(A,x_opt)-b))
...
ax_cvg.legend(prop={'size':5},loc=1)
ax_bup.legend(prop={'size':5},loc=4)

ylim = ax_cvg.get_ylim()
xlim = ax_cvg.get_xlim()

ax_cvg.text(xlim[1]*0.75, ylim[0]+np.log(0.15*ylim[1]), "eps="+str(eps)+", decay_rate="+str(decay_rate)+"\n" + \
                    "main_iter="+str(main_iter)+",int_iter="+str(1e3)+"\n"+ \
                    "cond_num="+str(cond_num)+", n="+str(n), size=5, \
                    va="baseline", ha="right", multialignment="left",
                    bbox=dict(fc="none"))
fig.tight_layout()
plt.show()
fpath = "../test_results/Spectrum/"+str(cond_num)+"_"+str(n)+"x"+str(n)+"/"
if not os.path.exists(fpath):
    os.makedirs(fpath)
u = datetime.datetime.fromtimestamp( time.time() ).strftime('%Y-%m-%d_%H.%M.%S')
fname = fpath+str(t)+"_"+str(u)+"_resids.png"
fig.savefig(fname)
